...
public class ItemMantenimientoHistorial extends VBox {
MantenimientoHistorial data;

    public ItemMantenimientoHistorial(MantenimientoHistorial data) {
        this.data = data;
        init();
    }
    ...
    private void init(){
        ...
        Label fechaLabel = new Label(data.getFecha_programada());
        Label estado = new Label(data.getEstado());
        estado.setStyle(Util.getStyleByState(data.getEstado()));


        fechaLabel.setStyle(styleTextDate);
        getChildren().addAll(fechaLabel, estado);
